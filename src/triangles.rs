...
#[derive(Debug, Clone)]
pub struct Triangle(pub Vec<Box<[[f64; 3]; 3]>>);

impl SVONode for Triangle{
    type Row = Box<[[f64; 3]; 3]>;
    ...
    fn entities_distribute(&self, bounds: [[f64; 3]; 2])->Vec<Self> {
        ...
        let mut child_list = vec![Triangle(Vec::new()); 8];
        let triangles_cloud = self.0.clone();
        for triangle in triangles_cloud{
            ...
            for i in 0..8{
                ...
                for edge in edges{
                    if line_intersect(edge, min, max){
            ...
            for index in index_array{
                child_list[index].0.push(triangle.clone());
            }           
        }
        child_list
    }
